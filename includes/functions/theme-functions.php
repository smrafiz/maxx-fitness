...
if ( ! function_exists( 'mfit_the_page_title' ) ) {
	...
	function mfit_the_page_title() {
		...
		if ( is_home() ) {
			...
			$title = __( 'Search results for', 'maxx-fitness' ) . ' "' . get_search_query() . '"';
		} elseif ( is_404() ) {
			$title = __( 'Page Not Found', 'maxx-fitness' );
		} else {
			global $post;
...
if ( ! function_exists( 'mfit_classic_pagination' ) ) {
	...
	function mfit_classic_pagination() {
		...
		$prev = esc_html_x( '&laquo; Older Posts', 'Older Posts', 'maxx-fitness' );
		$next = esc_html_x( 'Newer Posts &raquo;', 'Newer Posts', 'maxx-fitness' );
...
if ( ! function_exists( 'mfit_post_pagination' ) ) {
	...
	function mfit_post_pagination() {
		...
		$prev = esc_html_x( '&laquo;', 'Previous', 'maxx-fitness' );
		$next = esc_html_x( '&raquo;', 'Next', 'maxx-fitness' );
...
if ( ! function_exists( 'mfit_comments_meta' ) ) {
	...
	function mfit_comments_meta() {
		...
		if ( ! post_password_required() && ( comments_open() || 0 !== intval( get_comments_number() ) ) ) {
			$comments_number = get_comments_number_text( esc_html__( 'Click to Comment', 'maxx-fitness' ), esc_html__( '1 Comment', 'maxx-fitness' ), esc_html__( '% Comments', 'maxx-fitness' ) );

			$comments = sprintf(
				'<span class="comments-link"><span class="screen-reader-text">%1$s</span><a href="%2$s">%3$s</a></span>',
				esc_html__( 'Posted comments', 'maxx-fitness' ),
...
if ( ! function_exists( 'mfit_posted_in' ) ) {
	...
	function mfit_posted_in() {
		...
		return sprintf(
			...
			esc_html__( 'Posted in', 'maxx-fitness' ),
			$categories
		);
	}
}
...
if ( ! function_exists( 'mfit_comment_form' ) ) {
	...
	function mfit_comment_form() {
		...
		$author = '<div class="row">' .
					...
					'<fieldset>' .
					'<input id="author" name="author" type="text" placeholder="' . esc_html__( 'Name', 'maxx-fitness' ) . ( $req ? ' *' : '' ) . '" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' />' .
					'</fieldset>' .
					'</div>' .
					'</div>';
		...
		$email = '<div class="col-12 col-sm-12 col-md-4">' .
					...
					'<fieldset>' .
					'<input id="email" name="email" ' . ( $html5 ? 'type="email"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" placeholder="' . esc_html__( 'Email', 'maxx-fitness' ) . ( $req ? ' *' : '' ) . '" ' . $aria_req . ' />' .
					'</fieldset>' .
					'</div>' .
					'</div>';
			...
			<div class="comment-body">
				<div class="comment-media d-flex">
					...
					if ( ! empty( get_avatar( $comment ) ) ) {
						...
						<div class="comment-author vcard d-none d-sm-block">
							...
							echo get_avatar(
								...
								$size    = '128',
								$default = '',
								$alt     = sprintf( '%1$s %2$s', esc_html__( 'Avatar for', 'maxx-fitness' ), get_comment_author() )
							);
							?>
						</div><!-- .comment-author -->
						<?php
					...
					<div class="comment-content">
						...
						// Comment Author link.
						printf(
							wp_kses_post( '<cite class="fn">%s</cite>', 'maxx-fitness' ),
						...
						if ( '0' === $comment->comment_approved ) {
							?>
							<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'maxx-fitness' ); ?></em>
							<br />
							<?php
						}
						?>
						...
						<div class="comment-meta commentmetadata flex-wrap">
							<a href="<?php echo esc_url( htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ); ?>">
								<time datetime="<?php echo esc_attr( get_the_time( 'c' ) ); ?>">
									...
									printf( esc_html__( '%1$s at %2$s', 'maxx-fitness' ), get_comment_date(), get_comment_time() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									?>
								</time>
							</a>
							<?php
							edit_comment_link( esc_html__( '(Edit)', 'maxx-fitness' ), '  ', '' );
							?>
