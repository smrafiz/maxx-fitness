...
class Mfit_Breadcrumbs {
	...
	public function get_breadcrumbs( array $args ) {
		...
		$defaults = array(
			...
			'home_text'                 => sprintf( '<i class="fa fa-home"></i> <span>%s</span>', esc_attr__( 'Startseite', 'maxx-fitness' ) ),
			'cat_archive_prefix'        => esc_attr__( 'Category:&nbsp;', 'maxx-fitness' ),
			'tag_archive_prefix'        => esc_attr__( 'Tag:&nbsp;', 'maxx-fitness' ),
			'search_prefix'             => esc_attr__( 'Search query for:&nbsp;', 'maxx-fitness' ),
	...
	private function start_breadcrumb_rendering() {
		...
		// Woocommerce support.
		// if ( class_exists( 'WooCommerce' ) && ( ( Mfit_helpers::inside_woocommerce() && is_archive() && ! is_shop() ) || is_cart() || is_checkout() || is_account_page() || Mfit_Helpers::inside_product_cat() ) ) {
		...
		if ( is_singular() ) {
			...
			} elseif ( ( is_tax() || is_tag() || is_category() || is_date() || is_author() ) && $this->display_post_type_archive && ! Mfit_helpers::inside_woocommerce() ) {
				$this->render_markup .= $this->get_post_type_archive();
			}
			...
			if ( is_post_type_archive() ) {
				...
				// 404 Page.
				if ( Mfit_helpers::inside_tribe_event() || ( is_post_type_archive( 'tribe_events' ) || ( Mfit_helpers::inside_tribe_event() && is_archive() ) ) ) {
	...
	private function get_terms() {
		...
		if ( 'post' == $this->post->post_type ) {
			$taxonomy = 'category';
		} elseif ( 'product' == $this->post->post_type && class_exists( 'WooCommerce' ) && Mfit_Helpers::inside_woocommerce() ) {
	...
	private function get_breadcrumb_trail_markup( $object_type = '' ) {
		...
		switch ( $object_type ) {
			...
			case 'year':
				...
				$title = esc_html__( 'Year: ', 'maxx-fitness' ) . $year;
				break;
			case 'month':
				...
				$title = esc_html__( 'Month: ', 'maxx-fitness' ) . $wp_locale->get_month( $monthnum );
				break;
			case 'day':
				...
				$title = esc_html__( 'Day: ', 'maxx-fitness' ) . $day;
				break;
			case 'author':
				...
				$title = esc_html__( 'Articles Posted by&nbsp;', 'maxx-fitness' ) . $user->display_name;
				break;
			case 'search':
